...
export function AircraftHoldMap({ aircraft, allocation }: AircraftHoldMapProps) {
  ...
  return (
    <div className="bg-slate-50 dark:bg-white/5 backdrop-blur-md rounded-xl p-4 border border-slate-200 dark:border-white/10 mb-6 overflow-hidden">
      <h3 className="text-[10px] font-bold uppercase tracking-widest text-slate-500 dark:text-slate-400 mb-4 border-b border-slate-200 dark:border-white/10 pb-2">
      ...
      <div className="relative flex flex-col items-center justify-center py-4 w-full">
        ...
        <div className="hidden md:block absolute inset-0 w-full h-full opacity-20 pointer-events-none">
           <svg viewBox="0 0 800 200" className="w-full h-full" preserveAspectRatio="none">
             <path d="M 50,100 C 50,50 150,20 300,20 L 600,20 C 750,20 780,80 780,100 C 780,120 750,180 600,180 L 300,180 C 150,180 50,150 50,100 Z" 
                   fill="none" stroke="currentColor" strokeWidth="2" className="text-slate-400 dark:text-slate-600" />
             <path d="M 350,20 L 350,180" stroke="currentColor" strokeWidth="1" strokeDasharray="5,5" className="text-slate-300 dark:text-slate-700" />
           </svg>
        </div>
        ...
        <div className="relative z-10 flex flex-col md:flex-row items-center justify-between w-full max-w-lg px-2 gap-6 md:gap-2">
          ...
          <div className="flex flex-col items-center gap-2 w-full md:w-auto">
            <div className="text-[10px] font-mono text-slate-500 md:hidden tracking-widest border-b border-slate-300 dark:border-slate-700/50 pb-1 w-full text-center mb-1">PORÃO DIANTEIRO (FWD)</div>
          ...
          <div className="flex items-center justify-center px-1 w-full md:w-auto">
            ...
            <div className="hidden md:flex flex-col items-center">
              <div className="w-px h-8 bg-slate-300 dark:bg-slate-700/50"></div>
              <span className="text-[6px] text-slate-500 dark:text-slate-600 font-mono my-1">WING</span>
              <div className="w-px h-8 bg-slate-300 dark:bg-slate-700/50"></div>
            </div>
            ...
            <div className="md:hidden w-full flex items-center gap-2 opacity-50">
              ...
              <span className="text-[8px] text-slate-500 dark:text-slate-600 font-mono uppercase">Asas (Wing Box)</span>
              <div className="h-px bg-slate-300 dark:bg-slate-700 flex-1"></div>
            </div>
          </div>
          ...
          <div className="flex flex-col items-center gap-2 w-full md:w-auto">
            ...
            <div className="flex gap-1.5 sm:gap-2">
              ...
              {config.bulk > 0 && (
                <>
                  <div className="w-px h-10 border-l border-slate-300 dark:border-slate-700/50 border-dashed mx-0.5"></div>
                  {Array.from({ length: Math.max(config.bulk, allocation.bulk) }).map((_, i) => {
                     const isUsed = i < allocation.bulk;
                     const isOver = i >= config.bulk;
                     
                     let boxClass = 'bg-slate-200 dark:bg-slate-800/80 border-[#1b0088]/30 text-slate-500 dark:text-slate-400';
                     let label = 'LIV';
                     ...
                     if (isOver) {
                       ...
                       boxClass = 'bg-[#1b0088]/20 border-[#1b0088] text-slate-900 dark:text-white shadow-[0_0_10px_rgba(27,0,136,0.2)]';
                       label = 'BLK';
                     } else {
                       // Empty Bulk Position - Distinct border to identify it's BULK
                       boxClass = 'bg-slate-200 dark:bg-slate-800/80 border-[#1b0088]/50 text-slate-400 dark:text-white/40';
                       label = 'BLK';
                     }
              ...
              <div className="text-[8px] font-mono text-slate-500 dark:text-slate-400 hidden md:block self-center ml-1">TAIL</div>
            </div>
          </div>
        </div>
      </div>
      
      <div className="flex flex-wrap items-center justify-center gap-4 mt-4 pt-3 border-t border-slate-200 dark:border-white/10 px-4">
        ...
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-[9px] text-slate-500 dark:text-slate-400 font-mono">Bagagem</span>
        </div>
        <div className="flex items-center gap-1.5">
          <div className="w-2.5 h-2.5 rounded-sm bg-[#1b0088]/40 border border-[#1b0088]"></div>
          <span className="text-[9px] text-slate-500 dark:text-slate-400 font-mono">Bulk</span>
        </div>
        <div className="flex items-center gap-1.5">
          <div className="w-2.5 h-2.5 rounded-sm bg-red-500/40 border border-red-500 border-dashed"></div>
          <span className="text-[9px] text-slate-500 dark:text-slate-400 font-mono">Excesso</span>
        </div>
        <div className="flex items-center gap-1.5">
          <div className="w-2.5 h-2.5 rounded-sm bg-slate-200 dark:bg-white/5 border border-slate-300 dark:border-white/10"></div>
          <span className="text-[9px] text-slate-500 dark:text-slate-400 font-mono">Livre</span>
        </div>
      </div>
    </div>
  );
